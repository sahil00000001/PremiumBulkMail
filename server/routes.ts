...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Email tracking pixel endpoint (legacy)
  ...
  // so we only start the WS server in non-Vercel environments.
  // The client should fall back to polling /api/visitors/stats when WS is unavailable.
  if (!process.env.VERCEL) {
    ...
    wss.on("connection", (ws) => {
      ...
      visitorTrackingService.getVisitorStats().then((stats) => {
        ws.send(JSON.stringify({ type: "visitor_stats", data: stats }));
      });
    ...
    // Broadcast visitor stats every 5 seconds to all connected clients
    setInterval(async () => {
      if (wss.clients.size === 0) return;
      try {
        const stats = await visitorTrackingService.getVisitorStats();
        const message = JSON.stringify({ type: "visitor_stats", data: stats });
